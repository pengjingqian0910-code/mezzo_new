                ...
                <div class="flex-1 overflow-y-auto">
                    ...
                    <div v-for="geo in store.geofences" :key="geo.id" class="mb-3 p-3 bg-gray-900 rounded border border-gray-700">
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <button @click="toggleGeo(geo)" :class="geo.is_enabled ? 'bg-green-600' : 'bg-gray-600'" class="px-2 py-1 text-[10px] text-white rounded font-bold">
                                {{ geo.is_enabled ? '啟用中' : '已停用' }}
                            </button>
                        </div>
                        <div class="flex justify-end gap-2">
                            <button @click="renameGeo(geo)" class="text-blue-400 hover:text-white text-xs px-2 py-1 border border-blue-800 rounded">改名</button>
    ...
    setup() {
        ...
        const initDrawMap = () => {
            ...
            handler.setInputAction(async (click) => {
                ...
                if (geoName) {
                    await store.authFetch(`${BASE}/api/geofences`, {
        ...
        const toggleGeo = async (geo) => {
            await store.authFetch(`${BASE}/api/geofences/${geo.id}`, {
                method: 'PUT', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ is_enabled: !geo.is_enabled })
            });
            store.fetchGeofences();
        };
        const deleteGeo = async (id) => {
            if (confirm("確定刪除此警戒區？")) {
                await store.authFetch(`${BASE}/api/geofences/${id}`, { method: 'DELETE' });
                store.fetchGeofences();
            }
        };
        const renameGeo = async (geo) => {
            const newName = prompt("請輸入新的警戒區名稱：", geo.name);
            if (newName && newName !== geo.name) {
                await store.authFetch(`${BASE}/api/geofences/${geo.id}`, {
                    method: 'PUT', headers: { 'Content-Type': 'application/json' },
        ...
        return { store, isDrawing, startDrawing, toggleGeo, deleteGeo, renameGeo };
    }
};